...
export class SyncService {
    ...
    constructor() {
        const wpBaseUrl = process.env.WP_API_BASE_URL || 'https://thinhgialand.com/wp-json/wp/v2';
    ...
    transformPropertyToWordPress(property: Property): WordPressPost {
        const wpPost: WordPressPost = {
            ...
            status: this.mapStatus(property.status, property.web, property.public_approval) || 'draft',
            title: {
                raw: property.title || '',
            },
            content: {
        ...
        // House directions (huong-nha) - from house_directions array
        if (property.house_directions && property.house_directions.length > 0) {
            wpPost['huong-nha'] = property.house_directions;
        }

        // Balcony directions (huong-ban-cong) - from house_balcony_directions array
        if (property.house_balcony_directions && property.house_balcony_directions.length > 0) {
            wpPost['huong-ban-cong'] = property.house_balcony_directions;
        }
    ...
    private buildContent(property: Property): string {
        ...
        if (property.initialDescription) {
            parts.push(`<div class="initial-description">${property.initialDescription}</div>`);
        }

        // Internal description (might want to conditionally include)
        if (property.internalDescription && property.web) {
            parts.push(`<div class="internal-description">${property.internalDescription}</div>`);
        }

        // Property details as structured content
        const details: string[] = [];
        if (property.area) details.push(`<strong>Diện tích:</strong> ${property.area} m²`);
        if (property.floor_area) details.push(`<strong>Diện tích sàn:</strong> ${property.floor_area} m²`);
        if (property.into_money) details.push(`<strong>Giá:</strong> ${this.formatPrice(property.into_money)}`);
        if (property.number_bedroom) details.push(`<strong>Phòng ngủ:</strong> ${property.number_bedroom}`);
        if (property.number_wc) details.push(`<strong>Phòng tắm:</strong> ${property.number_wc}`);
        if (property.floors) details.push(`<strong>Số tầng:</strong> ${property.floors}`);
        if (property.house_address) details.push(`<strong>Địa chỉ:</strong> ${property.house_address}`);

        if (details.length > 0) {
            parts.unshift(`<div class="property-details"><ul><li>${details.join('</li><li>')}</li></ul></div>`);
        }
    ...
    async attachMediaToPost(mediaId: number, postId: number): Promise<void> {
        try {
            ...
            console.error(`Error attaching media ${mediaId} to post ${postId}:`, error);
            if (axios.isAxiosError(error)) {
     ...
     */
    async uploadAllPropertyMedia(property: Property): Promise<{ mediaIds: number[]; featuredId: number | null }> {
    ...
    async syncPropertyToWordPress(property: Property): Promise<any> {
        ...
        const { mediaIds, featuredId } = await this.uploadAllPropertyMedia(property);
        if (featuredId) {
        ...
        if (postId && mediaIds.length > 0) {
            for (const mediaId of mediaIds) {
                try {
                    ...
                    console.warn(`Could not attach media ${mediaId} to post ${postId}, skipping.`, e);
                }
            }
        }
    ...
    async processSyncJob(jobId: string): Promise<void> {
        try {
            ...
            if (job.type === 'single_property') {
                ...
                const properties = (job.payload as { properties: Property[] }).properties;
                let successCount = 0;
                let failedCount = 0;
                ...
                for (const property of properties) {
                    try {
                        ...
                        await this.syncJobService.updateProgress(jobId, successCount, failedCount, results);
                    } catch (error) {
                        failedCount++;
                        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
                        errors.push({
                        ...
                        results.push({ propertyId: property.id, success: false, error: errorMessage });
                        // Update progress periodically
                        await this.syncJobService.updateProgress(jobId, successCount, failedCount, results);
                    }
                }
                ...
                if (failedCount === 0) {
                    await this.syncJobService.markAsSuccess(jobId, { results, errors }, successCount);
                } else if (successCount === 0) {
                    await this.syncJobService.markAsFailed(jobId, `All properties failed. Errors: ${JSON.stringify(errors)}`);
                } else {
